...
export default function AdminPage() {
  ...
  // Member 4 Logic: Only SuperAdmin can perform ADM_USER actions based on Matrix 3.2
  const isSuperAdminAuth = currentUser?.user_type === 'SUPERADMIN'
  ...
  async function handleActivate(userId, username) {
    if (!isSuperAdminAuth) return; // Security Guard
    setActing(userId)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-surface-500 mt-0.5">
          Access Restricted: Only SuperAdmins can manage user accounts (Matrix 3.2).
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {filtered.map(u => {
                const isTargetSuper = u.user_type === 'SUPERADMIN'
                const isActingOn = acting === u.userId
                
                // Member 4 Matrix 3.2 Enforcement:
                // Only a SUPERADMIN can act on any row.
                // AND even a SUPERADMIN cannot deactivate their own or another SUPERADMIN account.
                const canModify = isSuperAdminAuth && !isTargetSuper

                return (
                  <tr key={u.userId} className={isTargetSuper ? 'bg-blue-50/30' : ''}>
                    <td className="font-mono text-xs text-surface-400">{u.userId?.slice(0, 8)}…</td>
                    <td className="font-medium">{u.username || '(no name)'}</td>
                    <td>
                    ...
                    <td>
                      {!isSuperAdminAuth ? (
                        <span className="text-xs text-surface-400 italic">No Permission</span>
                      ) : isTargetSuper ? (
                        <span className="text-xs text-indigo-600 font-bold italic">🛡️ Protected</span>
                      ) : u.record_status === 'ACTIVE' ? (
                        <button
                          className="btn-danger btn-sm"
                          onClick={() => handleDeactivate(u.userId, u.username)}
                          disabled={!canModify || isActingOn}
                        >
