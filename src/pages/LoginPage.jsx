...
export default function LoginPage() {
  const { signIn, signInWithGoogle, authError, currentUser } = useAuth()
  ...
  // Once AuthContext sets currentUser, redirect to /sales
  useEffect(() => {
  ...
  async function handleSubmit(e) {
    ...
    // On success: onAuthStateChange fires SIGNED_IN → sets currentUser → useEffect above redirects
  }
    ...
    <div className="min-h-screen bg-surface-100 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-surface-500 mt-0.5">Sales Management System</p>
        </div>

        <div className="bg-white rounded-2xl border border-surface-200 shadow-card-md p-7">
          <h2 className="text-base font-semibold text-surface-900 mb-5">Sign in</h2>
        ...
        <p className="text-center text-sm text-surface-500 mt-5">
          No account?{' '}
          <Link to="/register" className="text-surface-900 font-semibold hover:underline
                                          underline-offset-2">
            Create one
          </Link>
        </p>
